...
- The physical schema ships as `database/schema/001_graph_foundation_schema.sql`: everything lives in the dedicated `plmiqdb` schema with tables `foundation_vertex`, `foundation_edge`, and `foundation_graph_rule`; row-level-security policies keyed on the `app.tenant_id` session setting; and database-autoincremented `version` columns. Sample data lives in `database/seed/001_graph_foundation_seed.sql` (5 rows per table, FK-consistent), applied via `deploy-schema.bat -seed`.
