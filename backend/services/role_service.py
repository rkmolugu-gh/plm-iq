"""RoleService - roles, the permission catalog, grants, and user-role
assignments (iam_role / iam_permission / iam_role_permission / iam_user_role).

Why this class exists
---------------------
Isolation model (002), enforced in one place:
...
Benefits
--------
* The "exactly one role per user" business rule lives on one method
  (assign_roles_to_user) instead of being re-discovered at every call site.
* Grants/revokes return the post-change state, so routes stay one-liners.

How to extend (future scenarios)
--------------------------------
* Permission dependency graphs (require X implies Y) -> extend _resolve_permissions.
* Role hierarchies / edition-scoped bundles -> extend list()/create() filters.
"""
...
class RoleService(BaseService):
    ...
    def find(self, session: Session, role_id: UUID, *, tenant_id: UUID | None = None) -> dict | None:
    ...
    def get(self, session: Session, role_id: UUID, *, tenant_id: UUID | None = None) -> dict:
        role = self.find(session, role_id, tenant_id=tenant_id)
        if role is None:
    ...
    def list(
        ...
        *,
        ...
        total = session.execute(
            select(func.count()).select_from(tables.iam_role).where(*conditions)
        ).scalar_one()
        ...
        return Page[RoleOut](items=[from_row(RoleOut, r) for r in rows],
                             total=total, limit=limit, offset=offset)

    def create(self, session: Session, tenant_id: UUID, data: RoleCreate, actor: str) -> RoleOut:
        ...
        try:
            row = session.execute(
                insert(tables.iam_role).values(**values).returning(*tables.iam_role.c)
            ).one()
        ...
        logger.info("iam_role.created", extra={
            "tenant": str(tenant_id), "role": str(row.id), "actor": actor,
        })
        return from_row(RoleOut, row)

    def update(
        self, session: Session, tenant_id: UUID, role_id: UUID, data: RoleUpdate, actor: str
    ) -> RoleOut:
        current = self.get(session, role_id, tenant_id=tenant_id)
        if current["version"] != data.version:
            raise Conflict(
                f"version mismatch on role {role_id}: expected {data.version}, "
        ...
        if row is None:
            refreshed = self.find(session, role_id)
            if refreshed is None:
                raise NotFound(f"role {role_id} not found or not visible to this tenant")
            raise Conflict(
                f"concurrent modification on role {role_id}: expected version "
                f"{data.version}, current {refreshed['version']}"
            )
        logger.info("iam_role.updated", extra={
            "tenant": str(tenant_id), "role": str(role_id), "actor": actor,
        })
        return from_row(RoleOut, row)

    def delete(self, session: Session, tenant_id: UUID, role_id: UUID, actor: str) -> None:
        current = self._require_own_tenant_role(tenant_id, self.get(session, role_id, tenant_id=tenant_id))
        if current["is_system"]:
            raise Forbidden(f"system role '{current['code']}' cannot be deleted")
        deleted = session.execute(
            delete(tables.iam_role).where(tables.iam_role.c.id == role_id)
        ).rowcount
        if not deleted:
            raise NotFound(f"role {role_id} not found or not visible to this tenant")
        logger.info("iam_role.deleted", extra={
            "tenant": str(tenant_id), "role": str(role_id), "actor": actor,
        })
    ...
    def list_permissions(
        ...
        *,
        ...
        return Page[PermissionOut](items=[from_row(PermissionOut, r) for r in rows],
                                   total=total, limit=limit, offset=offset)

    def find_permission_by_id(self, session: Session, permission_id: UUID) -> dict | None:
        return self._fetch_one(session, tables.iam_permission, row_id=permission_id)

    def get_permission(self, session: Session, permission_id: UUID) -> dict:
        permission = self.find_permission_by_id(session, permission_id)
        if permission is None:
            raise NotFound(f"permission {permission_id} not found")
        return permission

    def create_permission(self, session: Session, data: PermissionCreate, actor: str) -> PermissionOut:
        try:
            row = session.execute(
    ...
    def update_permission(
        self, session: Session, permission_id: UUID, data: PermissionUpdate, actor: str
    ) -> PermissionOut:
        current = self.get_permission(session, permission_id)
    ...
    def delete_permission(self, session: Session, permission_id: UUID) -> None:
        self.get_permission(session, permission_id)
    ...
    def find_permission_by_code(self, session: Session, code: str) -> dict | None:
        row = session.execute(
    ...
    def grant_role_permissions(
        self, session: Session, tenant_id: UUID, role_id: UUID, codes: list[str], actor: str
    ) -> list[PermissionOut]:
        ...
        """
        self.get(session, role_id, tenant_id=tenant_id)
        permissions = self._resolve_permissions(session, codes)
        if permissions:
            session.execute(
        ...
        logger.info("iam_role.granted", extra={
            "tenant": str(tenant_id), "role": str(role_id), "codes": sorted(codes), "actor": actor,
        })
        return self.list_role_permissions(session, tenant_id, role_id)
    ...
    def revoke_role_permissions(
        ...
        self.get(session, role_id, tenant_id=tenant_id)
        permission_ids = [p.id for p in self._resolve_permissions(session, codes)]
        if permission_ids:
            session.execute(
        ...
        logger.info("iam_role.revoked", extra={
            "tenant": str(tenant_id), "role": str(role_id), "codes": sorted(codes), "actor": actor,
        })
        return self.list_role_permissions(session, tenant_id, role_id)

    def list_role_permissions(self, session: Session, tenant_id: UUID, role_id: UUID) -> list[PermissionOut]:
    ...
    def assign_roles_to_user(
        self, session: Session, tenant_id: UUID, user_id: UUID, role_ids: list[UUID], assigned_by: str
    ) -> list[RoleOut]:
        """Set THE role of a user - a user holds exactly one role.

        Passing more than one role id is rejected. The call replaces any
        existing assignment (delete-then-insert), so re-assignment stays atomic.
        """
        users.get(session, tenant_id, user_id)
        if len(role_ids) != 1:
            raise ValidationFailed("a user can hold only one role; pass exactly one role id")
        role = self.get(session, role_ids[0], tenant_id=tenant_id)
        session.execute(
        ...
        logger.info("iam_user.role_assigned", extra={
            "tenant": str(tenant_id), "user": str(user_id), "roles": [str(role["id"])],
        })
        return self.list_user_roles(session, tenant_id, user_id)

    def unassign_roles_from_user(
        self, session: Session, tenant_id: UUID, user_id: UUID, role_ids: list[UUID], actor: str
    ) -> list[RoleOut]:
        session.execute(
            delete(tables.iam_user_role).where(
                tables.iam_user_role.c.user_id == user_id,
        ...
        logger.info("iam_user.roles_unassigned", extra={
            "tenant": str(tenant_id), "user": str(user_id),
            "roles": [str(r) for r in role_ids], "actor": actor,
        })
        return self.list_user_roles(session, tenant_id, user_id)

    def list_user_roles(self, session: Session, tenant_id: UUID, user_id: UUID) -> list[RoleOut]:
        users.get(session, tenant_id, user_id)
        rows = session.execute(
            select(tables.iam_role)
    ...
    def roles_by_user(self, session: Session, tenant_id: UUID) -> dict[str, list[str]]:
    ...
    def effective_permissions(self, session: Session, tenant_id: UUID, user_id: UUID) -> list[str]:
        """Distinct permission codes reachable through the user's role assignments.

        The API layer short-circuits ``is_tenant_admin`` users to the full
        catalog; this reports exactly what assignment rows grant. Both join
