"""BM25 keyword search over the per-tenant Elasticsearch indices.
...
Results are normalized into view-ready rows with deep links back into the
graph workspace.
"""
...
from . import db, index_service, tenant_service
from .errors import ServiceError, ValidationFailed
from .es_ingest_service import _request
...
def search(tenant_id: UUID, query: str, *, limit: int = _DEFAULT_LIMIT) -> dict:
    ...
    vertex_hits = _search_index(
        index_service.vertex_index_name(slug),
        text,
        fields=_VERTEX_FIELDS,
        size=limit,
    )
    edge_hits = _search_index(
    ...
    rows = [_vertex_row(hit) for hit in vertex_hits] + [_edge_row(hit) for hit in edge_hits]
...
def _search_index(index: str, text: str, *, fields: list[str], size: int) -> list[dict]:
    body = {
        "size": size,
        "query": {
            "multi_match": {
        ...
        "highlight": {
            ...
            "fields": {"name": {}, "description": {}, "source_display.text": {}, "target_display.text": {}},
        },
    }
    path = f"/{urllib.parse.quote(index)}/_search"
    status, payload = _request("GET", path, body=json.dumps(body).encode())
    if status != 200:
        raise ServiceError(f"elasticsearch search on '{index}' failed: HTTP {status} {payload.get('error', '')}")
    return [
        {
            "id": hit["_id"],
            "score": hit["_score"],
