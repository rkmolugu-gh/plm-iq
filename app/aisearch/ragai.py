...
def rag_answer(query: str, entity_type: Optional[str] = None, tenant_key: Optional[str] = None) -> dict:
    ...
    """
    t_start = time.time()
    logger.info(f"RAG query='{query}' entity={entity_type} tenant_key={tenant_key}")

    # Step 1: Retrieve relevant context via hybrid search
    search_result, context_parts, citations, t_retrieve_elapsed = _retrieve_context(query, entity_type, t_start, tenant_key=tenant_key)
...
def rag_answer_multimodal(
    ...
    """
    t_start = time.time()
    logger.info(f"RAG multimodal query='{query}' entity={entity_type} tenant_key={tenant_key} images={len(images)}")
...
def _retrieve_context(query: str, entity_type: Optional[str], t_start: float, tenant_key: Optional[str] = None):
